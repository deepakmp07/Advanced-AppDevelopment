...
export default function RegisterPage() {
  ...
  const handleRegister = async (event) => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:8080/api/registration/customer', {
        username: name,
        email: email,
        password: password,
        mobileNumber:number,
    ...
    <div className="register">
      <div className="wrapper">
        <form onSubmit={handleRegister}>
          ...
          <div className="input-box">
            <input type="number" name="name" placeholder="Mobile Number" value={number} onChange={handleNumberChange} required />
          </div>
          <div className="input-box">
